...
$sql="select distinct DE.tdenunciaid, DE.cexpedientesedi, DE.cexpedientepolicial, OFE.cdocumentoid, OFE.cnombres, OFE.capellidos,
...
if (!empty($nombre) and empty($apellido)) {
	$sql = $sql . " or OFE.cnombres like '%$nombre%'";
		}
if (!empty($apellido) and empty($nombre)) {
	$sql = $sql . " or OFE.capellidos like '%$apellido%'";
		}
if (!empty($nombre) and !empty($apellido)) {
	$sql = $sql . " or (OFE.cnombres like '%$nombre%' and OFE.capellidos like '%$apellido%') ";
		}
if (!empty($policial)){ $sql = $sql. " or (DE.cexpedientepolicial like '%$policial%')";
        }

$sql = $sql . " or OFE.cdocumentoid= '$identidad')";
...
echo "<th>Nombre del Ofendido</th>";
