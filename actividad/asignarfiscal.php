...
<head>
  ...
  <meta http-equiv="Content-Type" content="text/html; charset=utf-8">
    ...
    <script type="text/javascript">        
        function MostrarJefe(SubBandeja, Fila){ 
//            alert(BandejaUsuario);  alert(Fila); alert(SubBandeja);

            var SubBandejaUsuario= "<?php echo $objUsuario->getSubBandejaId(); ?>"; 
            var BandejaUsuario= "<?php echo $objUsuario->getBandejaId(); ?>";  
            if (SubBandejaUsuario== SubBandeja){ //es de la misma oficina o fiscalia
                cbo= document.getElementById("cboFiscal"+Fila);
                ...
                var RolJefe= "2";
                $.ajax({
                    data:       "Rol="+RolJefe+"&Ban="+BandejaUsuario+"&Sub="+SubBandejaUsuario,
                    type:       "POST",
                    datatype:   "json",
                    url:    "../funciones/ajax_ListaFiscales_Jefe_NoJefe.php",
                    error: function (XMLHttpRequest, textStatus, errorThrown){
                        alert("Error al cargar datos para las listas de actividad");
                    },
                    success: function(json_obj){ 
                        var jsonFiscal= JSON.parse(json_obj);
                        
                        //actividad
                        var Filas= jsonFiscal.length;
                        
                        for(i= 0; i < Filas; i++){                        
                            opt= document.createElement("option");
                            opt.text=  jsonFiscal[i].nombre + "," + jsonFiscal[i].apellido;
                            opt.id= "optFiscal"+Fila;
                            opt.value= jsonFiscal[i].identidad;
            ...
            else{ //es de otra fiscalia o seccion
                cbo= document.getElementById("cboFiscal"+Fila);
                var length = cbo.options.length;
                for (i = 0; i < length; i++) {
                    cbo.options[i] = null;
                ...
                var RolJefe= "16";
                $.ajax({
                    data:       "Rol="+RolJefe+"&Ban="+BandejaUsuario+"&Sub="+SubBandeja,
                    type:       "POST",
                    datatype:   "json",
                    url:    "../funciones/ajax_ListaFiscales_Jefe_NoJefe.php",
                    error: function (XMLHttpRequest, textStatus, errorThrown){
...
//            alert(Tabla+Fila+Fiscalia+Denunciado);
            ...
            fil.appendChild(col4);            
               ...
               //agregar evento para mostrar jefe o subalternos
                var evento= document.getElementById("cboOficina"+Fila);
                evento.addEventListener('change', function(){MostrarJefe(evento.value, Fila)});    
                //BandejaNueva, SubBandejaNueva, Fila
