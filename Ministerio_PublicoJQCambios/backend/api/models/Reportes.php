...
class Reportes extends Model
{
     ...
     public static function ActividadFiscal($usuario=null) {
		$sql="select 	a.ndelito,
					  i.cdescripcion delito,
					   a.thora,
					  to_char(a.dfecha ,'YYYY-MM-DD') fecharegistro,
					   ...
					   concat(h.cnombres,',',h.capellidos)cnombres,
					   concat(j.cnombres,',',h.capellidos)cnombresofendido,
					   h.crtn crtn,
					   h.tdenunciaid,
					   a.cobservacion cobservacion
	  ...
	  public static function ActividadFiscalFechas($usuario=null,$fechainicio=null,$fechafinal=null) {
		$sql="select 		  a.ndelito,
				   ...
				   where a.nmateriaid=b.nmateria and
					...
					a.dfecha  BETWEEN '$fechainicio' AND '$fechafinal'					
					order by f.dfechadenuncia Desc	";
		$Ireporte = new Reportes();
		return new Resultset(null, $Ireporte, $Ireporte->getReadConnection()->query($sql));
	}
	...
	public static function ConfirmacionDenuncia() {
			$sql='select  a.tdenunciaid,
					a.tpersonaid,
					concat(b.cnombres,",",b.capellidos)cnombres,
					concat(j.cnombres,","j.capellidos )nombresofendido,
					b.crtn,
					c.ndelito,
				...
				f.ibandejaid=g.ibandejaid and
				j.tdenunciaid=e.tdenunciaid and
	...
	public static function ConfirmacionDenunciaFiscal($fiscal=null) {
	$sql="select  a.tdenunciaid,
				a.tpersonaid,
				d.cusuario fiscalasiganado,
				concat(b.cnombres,',',b.capellidos)cnombres,
...
public static function VaciadoDenunciaTodosDelitos() {
	   ...
	   $sql="select a.tdenunciaid as tdenunciaid,
					...
					e.cdireccion direcciondenunciante,
					e.csexo sexodenunciante,
					e.cgenero generodenunciante,
					...
					c.cgenero generoimputado,
					c.iedad edadimputado,
					c.cdetalle direccionimputado,
					c.corientacionsexual orientacionsexualimputado,
					c.ctelefono telefonoimputado,
					c.aplicalgbti aplicalgbtiimputado,
					c.calias aliasimputado,
					c.cnombreasumido nombreasumidoimputado,
					c.ccondicion condisionimptado,
					c.ctrabajoremunerado trabajoremuneradoimputado,
					c.casisteeducacion asisteeducacionimputado,
					c.bmenorinfractor infractorimputado,
	   ...
	   where a.tdenunciaid=b.tdenunciaid and
		     ...
		     c.ndiscapacidadid=dis3.ndiscapacidadid and
		     c.nestadocivil=ec3.ncivil and
		     c.netniaid=g3.netniaid";
		$Ireporte = new Reportes();
		return new Resultset(null, $Ireporte, $Ireporte->getReadConnection()->query($sql));
	}

	
   	public static function VaciadoDenunciaTodosDelitosFechas($fechainicio=null,$fechafinal=null) {
	...
	//agregar usuario de seron
    {
        $sql="select *from mini_sedi.tbl_usuarios  where isubbandejaid=(
				Select isubbandejaid from mini_sedi.tbl_usuarios where usuario='$user') order by nombres asc";
			$result = new Reportes();
...
public static function FrecuenciaDenRecibidas()
	...
	public static function VaciadoGestionFiscal($usuario)
    {
			...
			$sql="select  mini_sedi.tbl_imputado_actividad_delito.tpersonaid personaid,
					...
					mini_sedi.tbl_imputado_actividad_delito.thora horagestion,
					mini_sedi.tbl_imputado_actividad_delito.dfecha fechadenuncia
