 ...
 */
 ...
 function BusquedaconsultarNuevo($fecha1, $fecha2)
{ 
   
    $sql= "SELECT
                 io.tdenunciaid, ban.cdescripcion as recepcion,  im.cnombres || ' ' || im.capellidos as imputado, im.csexo as sex_imp, of.cnombres || ' ' || of.capellidos as ofendido, of.cgenero as genero_ofen, 
                pa1.cdescripcion as parentesco_imp_ofen, del.cdescripcion, den.dfechadenuncia, dp.cdescripcion as depto, mu.cdescripcion as muni,
                oc.cdescripcion as ocupacion, ec.cdescripcion as estadocivil, esc.cdescripcion as escolaridad, of.iedad, of.cunidadmedidaedad
            FROM
             mini_sedi.tbl_imputado_ofendido as io
                inner join mini_sedi.tbl_imputado as im on io.tdenunciaid= im.tdenunciaid
                inner join mini_sedi.tbl_ofendido as of on io.tdenunciaid= of.tdenunciaid
                inner join mini_sedi.tbl_parentescos as pa1 on io.nparentescoid= pa1.nparentescoid
                inner join mini_sedi.tbl_imputado_delito as id on im.tpersonaid= id.tpersonaid
                inner join mini_sedi.tbl_delito as del on id.ndelito= del.ndelitoid
                inner join mini_sedi.tbl_denuncia as den on io.tdenunciaid= den.tdenunciaid
                inner join mini_sedi.tbl_departamentopais as dp on dp.cdepartamentoid= den.cdeptodenuncia
                inner join mini_sedi.tbl_municipio as mu on mu.cdepartamentoid= den.cdeptodenuncia and mu.cmunicipioid= den.cmunicipiodenuncia
                inner join mini_sedi.tbl_ocupacion as oc on oc.nocupacionid= of.nocupacionid
                inner join mini_sedi.tbl_estadoscivil as ec on ec.ncivil= of.nestadocivil
                inner join mini_sedi.tbl_escolaridad as esc on esc.nescolaridadid= of.nescolaridadid
                inner join mini_sedi.tbl_bandejas as ban on ban.ibandejaid= den.ibandejaid
            where 
                den.dfechadenuncia >= '$fecha1' and den.dfechadenuncia <= '$fecha2'
            order by 
            dfechadenuncia";  
...
/* FUNCION DAVID
 */
function Busquedaconsultar_imputado_arma($fecha1, $fecha2)
{ 
   
    $sql= "SELECT 
                a.tdenunciaid as n_denuncia, a.dfechadenuncia as fecha_denuncia, a.dfechahecho as fecha_hecho, 
                b.cdescripcion as depto_denuncia, c.cdescripcion as municipio_denuncia, d.cnombres || ' ' || d.capellidos as imputado, 
                d.iedad as edad_imputado, d.cgenero as sexo_imputado,
                d.cdocumentoid as documento_imputado, f.cdescripcion as delito, h.cdescripcion as arma
            FROM mini_sedi.tbl_denuncia a, mini_sedi.tbl_departamentopais b, mini_sedi.tbl_municipio c, mini_sedi.tbl_imputado d, mini_sedi.tbl_imputado_delito e,
                mini_sedi.tbl_delito f, mini_sedi.tbl_imputado_arma g, mini_sedi.tbl_tipo_arma h
            WHERE b.cdepartamentoid= a.cdeptodenuncia and c.cdepartamentoid= a.cdeptodenuncia and c.cmunicipioid= a.cmunicipiodenuncia
                and a.tdenunciaid= d.tdenunciaid and d.tpersonaid= e.tpersonaid and e.ndelito= f.ndelitoid
                and a.tdenunciaid= d.tdenunciaid and d.tpersonaid= g.tpersonaid and g.narmaid= h.narmaid
                and a.dfechadenuncia >= '$fecha1' and a.dfechadenuncia <= '$fecha2'
            order by 
            dfechadenuncia";  
